...
def simulation(n_samples, hazard_ratio, n_repeats=3):
    ...
    for cens in (0.1, 0.25, 0.4, 0.5, 0.6, 0.7):
        ...
        for _ in range(n_repeats):
            ...
            c_harrell = concordance_index_censored(y_test_split["event"].values, y_test_split["time"].values, X_test_split.flatten())
            c_uno = concordance_index_ipcw(y, Surv.from_arrays(y_test_split['event'].values, y_test_split['time'].values), X_test_split.flatten())
            ...
            grid_rsf = GridSearchCV(RandomSurvivalForest(random_state=0), param_grid, cv=3, scoring="neg_mean_absolute_error", return_train_score=True) #added return train score
            ...
            c_rsf_test = concordance_index_censored(y_test_split["event"].values, y_test_split["time"].values, risk_scores_test)
            ...
            data["Params and Results"].append(grid_rsf.cv_results_) #added line

        for key, values in data.items():
            if key != "Best Params" and key != "Params and Results":
    ...
    data_std = pd.DataFrame.from_dict({k: v for k, v in data_std.items() if k != "Best Params" and k != "Params and Results"})
...
def plot_hyperparameters(data_mean):
    results = []
    for params_and_results in data_mean["Params and Results"]:
        for i in range(len(params_and_results['params'])):
            params = params_and_results['params'][i]
            mean_test_score = params_and_results['mean_test_score'][i]
            params_str = str(params)
            results.append({"params": params_str, "mean_test_score": mean_test_score})

    results_df = pd.DataFrame(results)

    plt.figure(figsize=(12, 6))
    sns.barplot(x="params", y="mean_test_score", data=results_df)
